...
- **BR-16 – Vendor Pickup and Status Updates:** Vendors (6–10 donation/pickup partners) access the Vendor Portal to manage assigned pickups. The status lifecycle is: the ticket starts in **"Pickup"** status (customer created a pickup request). After the vendor physically collects the item, the vendor returns to the portal and **manually** updates the status to **"Received"**. This **"Received"** status change is the trigger for refund processing. **Note:** The transition from "Pickup" to "Received" is a manual vendor action, not automatic.
